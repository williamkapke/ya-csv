...
var CsvReader = csv.CsvReader = function(readStream, options) {
	...
	if (readStream) {
		readStream.addListener('data', ondata);
		readStream.addListener('error', onerror);
		readStream.addListener('end', onend);
...
CsvReader.prototype.parse = function(data) {
	...
	for (var i = 0; !this.aborted && i < data.length; i++) {
		var c = data.charAt(i);
		switch (c) {
			...
			case this.quotechar:
				...
				if (c === this.escapechar) {
					...
					if( !(this.escapechar === this.quotechar && !ps.openField && !ps.quotedField ) && this._isEscapable(nextChar) ) {
...
CsvReader.prototype._addRecord = function() {
	...
	if (this.columnsFromHeader && ps.rows === 0) {
		...
		for (var i = 0; i < this.columnNames.length; i++) {
			if(this.ignoreEmptyFields && !ps.openRecord[i]) continue;
...
var CsvWriter = csv.CsvWriter = function(writeStream, options) {
	...
	if(this.columnNames && this.columnNames.length)
		this.writeRecord(this.columnNames);
};
...
function _setOptions(obj, options) {
	...
	obj.ignoreEmptyFields = options.ignoreEmptyFields || false;
};
